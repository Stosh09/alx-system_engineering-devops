...
matching to try and locate the erroneous `.php` file extension. Located it in the
...
error in `wp-settings.php` when trying to load the file `class-wp-locale.php`. The correct
...
to automate fixing of any such identical errors should they occur in the future. The manifest
replaces any `php` extensions in the file `/var/www/html/wp-settings.php` with `php`.
